-- Ensure get_comments accepts p_comments_offset (used by the app) and returns likes_count.
-- Drop the 3-param overload so the 4-param version is the only one (and returns likes_count).
DROP FUNCTION IF EXISTS get_comments(INTEGER, INTEGER, INTEGER);
