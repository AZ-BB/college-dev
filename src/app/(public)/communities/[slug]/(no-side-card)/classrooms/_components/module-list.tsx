...
export function ModuleList() {
    ...
    return (
        <div className="w-1/4 space-y-6">
            ...
            {classroomData.modules.map((module, moduleArrayIndex) => (
                                ...
                                isReadOnly ? (
                                    <div className="space-y-2">
                                        {module.lessons.map((lesson) => (
                                            <div
                                                key={lesson.index}
                                                className="px-4 py-2 rounded-lg cursor-pointer hover:bg-grey-200"
                                                onClick={() => setSelectedLessonIndex(lesson.index)}
                                            >
                                    ...
                                    <DndContext
                                        ...
                                        onDragEnd={(event) => handleDragEnd(event, moduleArrayIndex)}
                                    >
                                        <SortableContext
                                            items={module.lessons.map(lesson => String(lesson.index))}
                                            strategy={verticalListSortingStrategy}
                                        >
                                            {module.lessons.map((lesson) => (
                                                <SortableLessonItem
                                                    key={`${module.index}-${lesson.index}`}
                                                    lesson={lesson}
