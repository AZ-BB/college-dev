...
export async function POST(request: NextRequest) {
    try {
        ...
        // Get the community to find the old cover image
        ...
        const filePath = `${commSlug}/cover.${fileExt}`;

        // Check if old cover exists and extract its file type
        let useUpsert = false;
        ...
        if (community.cover_image) {
            try {
                ...
                const urlParts = community.cover_image.split('/community_media/');
                ...
                console.error('Error checking/deleting old cover image:', error);
        ...
        // Update the community's cover_image column
        ...
        if (updateError) {
            console.error("Error updating community cover_image:", updateError);
        ...
        console.error('Error in upload cover API:', error);
        return NextResponse.json(
            { error: 'Failed to upload cover image' },
            { status: 500 }
        );
