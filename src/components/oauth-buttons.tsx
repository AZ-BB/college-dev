...
const getOAuthConfig = (provider: OAuthProvider, type: "signin" | "signup") => {
  ...
  const configs = {
    ...
    discord: {
      label: `${action} with Discord`,
      icon: (
        <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#5865F2">
...
export function OAuthButtons({ type, redirect }: OAuthButtonsProps) {
  ...
  if (config.oauth_types.length === 0) {
    return null
  }

  return (
    <>
      {config.oauth_types.map((provider) => {
        ...
        return (
          <Button
            ...
            className="h-12 hover:text-black hover:bg-[#21213a] font-instrumentSans w-full justify-center gap-3 rounded-2xl border border-transparent text-sm font-semibold text-[#1f1f1f] shadow-none transition cursor-pointer focus-visible:ring-[#f89b63]/40"
            aria-label={oauthConfig.label}
          >
            {oauthConfig.icon}
            {oauthConfig.label}
          </Button>
