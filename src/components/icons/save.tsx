import { cn } from "@/lib/utils";

export default function SaveIcon({ className }: { className?: string }) {
    return (
        <svg className={cn('stroke-[#0E1011]', className)} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12.89 5.88281H5.10999C3.39999 5.88281 2 7.2828 2 8.9928V20.3528C2 21.8028 3.04 22.4228 4.31 21.7128L8.23999 19.5228C8.65999 19.2928 9.34 19.2928 9.75 19.5228L13.68 21.7128C14.95 22.4228 15.99 21.8028 15.99 20.3528V8.9928C16 7.2828 14.6 5.88281 12.89 5.88281Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M16 8.9928V20.3528C16 21.8028 14.96 22.4128 13.69 21.7128L9.76001 19.5228C9.34001 19.2928 8.65999 19.2928 8.23999 19.5228L4.31 21.7128C3.04 22.4128 2 21.8028 2 20.3528V8.9928C2 7.2828 3.39999 5.88281 5.10999 5.88281H12.89C14.6 5.88281 16 7.2828 16 8.9928Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M22 5.10999V16.47C22 17.92 20.96 18.53 19.69 17.83L16 15.77V8.98999C16 7.27999 14.6 5.88 12.89 5.88H8V5.10999C8 3.39999 9.39999 2 11.11 2H18.89C20.6 2 22 3.39999 22 5.10999Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
    )
}